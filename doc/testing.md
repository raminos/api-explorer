...
The root `bun run check` command runs strict TypeScript, Biome, and the complete unit and integration suite. `bun run check:generated` generates the comprehensive showcase in a clean deterministic directory, installs its pinned dependencies, type-checks the server, and production-builds the web app. Non-zero child-command exit codes are typed verification failures. `bun run check:commit` runs both gates.
